export default class InitialScreen extends Phaser.Scene {
  ...
  create() {
    ...
    // 시작 안내 텍스트
    this.add.text(this.cameras.main.centerX, this.cameras.main.centerY + 20, 'Z: 시작 · Enter: 디버그 모드', {
      fontSize: '16px',
      fill: '#888888',
      fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace'
    }).setOrigin(0.5);
    ...
    // 키 입력 이벤트 리스너: Z → 일반, Enter → 디버그
    this.input.keyboard.on('keydown', (event) => {
      if (event.code === 'KeyZ') {
        this.scene.start('CharacterSelect', { debugMode: false });
