...
export default class HUD extends Phaser.GameObjects.Container {
  constructor(scene) {
    ...
    this.hpText = scene.add
      .text(0, 0, "", { fontSize: 12, color: "#ffffff" })
      .setDepth(15);
  ...
  relayout() {
    ...
    this.hpY = top;
    // 창 크기 변경 시에도 현재 플레이어 HP를 유지해 표시
    if (this.player) {
      this.drawHp(this.player.hp, this.player.maxHp);
    } else {
  ...
  _ensureSkillIcon(id, x, y, size) {
    if (!this.skillIcons[id]) {
      const icon = this.scene.add
        .rectangle(x + size / 2, y + size / 2, size, size, 0xaaaaaa)
  ...
  updateSkillMask(id, ratio, leftSec = 0) {
    ...
    if (this.skillTexts?.[id]) {
      this.skillTexts[id].setText(
        leftSec > 0 && ratio > 0 ? leftSec.toFixed(1) : ""
      );
    }
  }
  ...
  bind(player) {
    ...
    this._chargeHandler = ({
      ...
      if (this.skillStackTexts?.[slot]) {
        this.skillStackTexts[slot].setText(
          typeof charges === "number" ? String(charges) : ""
        );
      }
      // 리차지 오버레이
      const ratio =
        rechargeMax > 0
          ? Phaser.Math.Clamp((rechargeLeft ?? 0) / rechargeMax, 0, 1)
          : 0;
  ...
  _startDeathFlow() {
    ...
    this.scene.time.addEvent({
      ...
      callback: () => {
        left -= 1;
        txt.setText(String(left));
        if (left <= 0) {
          overlay.destroy();
          txt.destroy();
          this.player.hp = this.player.maxHp;
          this.player.events.emit("hp:changed", {
            hp: this.player.hp,
