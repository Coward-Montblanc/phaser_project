...
export function runDash(owner, opts) {
    ...
    if (cfg.wall.mode === 'block_all') {
      const scan = _scanCenterLastFree(layer, sx, sy, dirX, dirY, cfg.distance, stepPx);
    ...
    const growEvt = scene.time.addEvent({
      ...
      callback: () => {
        ...
        if (g) {
          ...
          const hw = width / 2, npx = -dirY, npy = dirX;
          g.beginPath();
    ...
    const endDash = () => {
      ...
      if (layer) {
        const pushed = _pushOutFromWalls(layer, owner.x, owner.y, cfg.hit.radius ?? 6, 14, 1.0);
        owner.setPosition(pushed.x, pushed.y);
      }

      // 이펙트 줄이며 제거 + 샘플 정리
      scene.tweens.addCounter({
        from: 1, to: 0, duration: 200, ease: 'Sine.easeIn',
        onUpdate: (tw) => {
          const p = tw.getValue();
          if (g) {
            g.clear();
            if (p > 0) {
              ...
              const nx = sx + dirX * curLen, ny = sy + dirY * curLen;
              const hw = width / 2, npx = -dirY, npy = dirX;
              g.beginPath();
              g.moveTo(sx + npx*hw, sy + npy*hw);
              g.lineTo(nx + npx*hw, ny + npy*hw);
          ...
          while (samples.length) {
            ...
            if (d <= targetDist * (1 - p)) { s.destroy(); samples.shift(); }
            else break;
          }
        },
        onComplete: () => {
    ...
    function _lineSpawnAlong(fromX, fromY, toX, toY, step, cb, startDist=0) {
      ...
      let d = startDist;
      while (d <= len) { cb(fromX + ux * d, fromY + uy * d, d); d += step; }
      return { last: d - step };
    }
    ...
    function _scanCenterLastFree(layer, sx, sy, dirX, dirY, maxDist, stepPx) {
        ...
        const x = sx + dirX * d, y = sy + dirY * d;
        if (!_isBlockedAtCenter(layer, x, y)) lastFree = d;
        if (d >= maxDist) break;
        }
        const destX = sx + dirX * maxDist, destY = sy + dirY * maxDist;
        ...
        for (let i = 0, d = 0; i <= steps2; i++, d = Math.min(maxDist, d + stepPx)) {
            const x = sx + dirX * d, y = sy + dirY * d;
            if (_isBlockedAtCenter(layer, x, y)) { passMid = true; break; }
            if (d >= maxDist) break;
        }
        }
    ...
    function _pushOutFromWalls(layer, x, y, pad, maxIter = 12, step = 1.0) {
        ...
        const hitRU = _isBlockedAtCenter(layer, cx + pad * 0.707, cy - pad * 0.707);
        const hitRD = _isBlockedAtCenter(layer, cx + pad * 0.707, cy + pad * 0.707);
        const hitLU = _isBlockedAtCenter(layer, cx - pad * 0.707, cy - pad * 0.707);
        const hitLD = _isBlockedAtCenter(layer, cx - pad * 0.707, cy + pad * 0.707);
    
        const any = hitR || hitL || hitD || hitU || hitRU || hitRD || hitLU || hitLD;
        if (!any) break;
        ...
        if (hitD) ny -= 1;
        if (hitU) ny += 1;
        if (hitRD) { nx -= 0.707; ny -= 0.707; }
        if (hitRU) { nx -= 0.707; ny += 0.707; }
        if (hitLD) { nx += 0.707; ny -= 0.707; }
        if (hitLU) { nx += 0.707; ny += 0.707; }
    ...
    return { stop: () => { try { growEvt.remove(false); } catch(_){} owner.isDashing=false; } };
  }
